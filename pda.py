# PDA Simulator for L = {0^n 1^m | m = 2n}

def read_pda_file(filename):
    pda = {}
    current = None
            ...
            if line.startswith('[') and line.endswith(']'):
                ...
                if current == 'sigma':
                    for symbol in tokens:
                        if symbol not in pda[current]:
                            pda[current].append(symbol)
                elif current:
                    pda[current].append(tokens)
    return pda
...
def is_valid_rule(rule, pda):
    ...
    return (
        ...
        all(sym in pda['sigma'] or sym == 'e' for sym in rule[1:4])
    )
